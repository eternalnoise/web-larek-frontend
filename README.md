...
### ICheckoutDetails
Композирует данные из форм `IDeliveryDetails`, `IBasket` и `IContacts`, собирая все данные для заказа в одном месте.
...
Отвечает за рендер отдельной карточки товара в каталоге. На самом элементе карточки висит обработчик событий, вызывающий событие `catalog_selected`. Каждая карточка собирается из интерфейса `IProduct`.
